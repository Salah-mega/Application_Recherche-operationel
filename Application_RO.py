...
# Fonction pour générer un graphe a condition 
def generate_strongly_connected_graph_with_weights():
    ...
    num_edges = simpledialog.askinteger("Entrée", "Entrez le nombre d'arêtes :", minvalue=num_nodes - 1)
    ...
    if num_edges < num_nodes - 1:
        messagebox.showerror("Erreur", "Le nombre d'arêtes doit être au moins égal à (nombre de sommets - 1).")
        return None
...
def display_graph_with_weights(graph, node_colors=None):
    ...
    graph_window.title("Visualisation du Graphe")
...
def open_transport_optimization():
    ...
    transport_window.title("Visualisation des tableaux")
...
def welsh_powell(update_results):
    ...
    if graph:
        ...
        result = f"Coleur des sommets : {coloring}"
        update_results(result)
...
def create_main_interface():
    title = tk.Label(root, text="Algorithmes Recherche Opérationnelle - 4e année IIR EMSI", font=("Arial", 18, "bold"), pady=20, fg="#222831")
    title.pack()
    
     # Cadre pour les mentions
    mentions_frame = tk.Frame(root, bg="#FF9D23", padx=10, pady=10, relief="ridge", borderwidth=2)
    mentions_frame.pack(pady=10, fill="x")  # Étendre le cadre horizontalement

    tk.Label(mentions_frame, text="Réalisé par : SAHNOUNE Salaheddine & JOUHLALI Abdeladim ", font=("Arial", 13, "bold"),
             fg="#212121", bg="#FF9D23").pack(pady=2)
    tk.Label(mentions_frame, text="Encadré par : Dr. EL MKHALET MOUNA ", font=("Arial", 13, "bold"),
             fg="#212121", bg="#FF9D23").pack(pady=2)
    ...
    algorithms = [
        ("Welsh Powell", lambda: welsh_powell(update_results)),
        ("Dijkstra", lambda: dijkstra(update_results)),
        ("Kruskal", lambda: kruskal(update_results)),
        ("Bellman-Ford", lambda: bellman_ford(update_results)),
        ("Ford-Fulkerson", lambda: ford_fulkerson(update_results)),
    ]
    ...
    # Création des boutons avec une disposition horizontale
    for idx, (algo_name, algo_function) in enumerate(algorithms):
        btn = tk.Button(button_frame, text=algo_name, font=("Verdana", 13), command=algo_function, bg="#FF9D23", fg="#212121", width=20, height=2)
        btn.grid(row=0, column=idx, padx=10, pady=10)  # Organisation en ligne avec espace entre les boutons
        
       # Boutons supplémentaires
    tk.Button(root, text="Les Algorithmes d'Optimisation", command=open_transport_optimization,
              bg="#FF9D23", fg="#212121", font=("Verdana", 13), width=30).pack(pady=20)

    tk.Button(root, text="Quitter", font=("Verdana", 12, "bold"), command=root.quit,
              bg="#212121", fg="#EB5A3C", width=30).pack(pady=5) 
    
# Fenêtre principale
root = tk.Tk()
root.title("Application des Algorithmes")
root.geometry("800x600")
image = tk.PhotoImage(file="C:/Users/pc/Projet_RO/RO.png")
